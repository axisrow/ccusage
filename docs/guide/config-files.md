...
Use data source namespaces to set defaults and report overrides. Supported namespaces are `claude`, `codex`, `opencode`, `amp`, `droid`, `codebuff`, `hermes`, `pi`, `goose`, `zcode`, `openclaw`, `kilo`, `kimi`, `qwen`, `copilot`, and `gemini`.
